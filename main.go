...
func dislikeHandler(w http.ResponseWriter, r *http.Request) {
	...
	if postID != "" {
		_, err = db.Exec("DELETE FROM likes WHERE user_id = ? AND post_id = ?", session.UserID, postID)
	} else if commentID != "" {
		_, err = db.Exec("DELETE FROM likes WHERE user_id = ? AND comment_id = ?", session.UserID, commentID)
	}
...
func viewPostHandler(w http.ResponseWriter, r *http.Request) {
	...
	var post Post
	var categories string
	err := db.QueryRow("SELECT id, user_id, title, content, categories, created_at FROM posts WHERE id = ?", postID).Scan(&post.ID, &post.UserID, &post.Title, &post.Content, &categories, &post.CreatedAt)
	if err != nil {
	...
	tmpl, _ := template.ParseFiles("templates/viewPost.html")
	tmpl.Execute(w, data)
}
